  ...
  CREATE TABLE IF NOT EXISTS public.package_option_groups (
    ...
    UNIQUE (company_id, package_id, option_group_key)
  );
  ...
  INSERT INTO public.package_option_groups (
    company_id, package_id, option_group_key, label_pt, min_choices, max_choices,
    required, blocks_additional_items, display_order
  )
  SELECT
    :'cdl_company_id'::uuid,
    p.id,
  ...
  ) AS g(option_group_key, label_pt, display_order)
  CROSS JOIN (
    SELECT id FROM public.packages
    WHERE company_id = :'cdl_company_id'::uuid
  ...
  ON CONFLICT (company_id, package_id, option_group_key) DO NOTHING;

  -- Prime: Salmão ou camarão
  INSERT INTO public.package_option_group_items (
    company_id, option_group_id, additional_item_id, option_item_key, label_pt, display_order
  )
  SELECT
    :'cdl_company_id'::uuid,
    og.id,
  ...
  CROSS JOIN (
    VALUES
      ('salmao', 'Salmão', 1),
      ('camarao', 'Camarão', 2)
  ) AS opt(option_item_key, label_pt, display_order)
  LEFT JOIN public.additional_items ai
    ON ai.company_id = :'cdl_company_id'::uuid
    AND lower(trim(ai.item_key)) = opt.option_item_key
  ...
  -- Choice: Salmão ou camarão (same option_group_key on BBQCHO packages)
